...
class RSA(Template,Encrypt,Decrypt):
    ...
    def findMaxCoprimeNumber(phi,e=2,auto=False):
        ...
        return [ x for x in range(2,phi) if RSA.egcd(phi,x)[0]==1][-1]#max([ x for x in range(2,phi) if RSA.egcd(phi,x)[0]==1 and x.is_integer()])
    @staticmethod
    def egcd(a, b):
        if a == 0:
            return (b, 0, 1)
    ...
    def findCoprime(self,phi):
        ...
        return self.findMaxCoprimeNumber(phi)



        
    def findComprimeD(self,phi,p) ->int:
        d = lambda k,phi,p: (1+(k*phi))/p
        pd:int = 2
        for x in range(0,phi):
            ...
            if pd.is_integer():
                return int(pd)
            continue
        return self.findComprimeD(phi,p)# si no se encontro uno
    def generateKey(self):
        ...
        self.PBPK_CONTAINER = KeysContainer(e,d,N)
